...
#if defined(NDEBUG)
#error "XVGDG cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uXVGDG) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight >= 101  && nHeight <= 300){

        nSubsidy = 500 * COIN;
}

	else if (nHeight >= 301  && nHeight <= 600){

        nSubsidy = 10 * COIN;
}

	else if (nHeight >= 601  && nHeight <= 1000){

        nSubsidy = 5 * COIN;
}

	else if (nHeight >= 1001  && nHeight <= 30000){
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
   ...
   // Rewards masternode activation. 
    
if (nHeight >=101 && nHeight <=300)
{
    ret = blockValue * 0.80; 
}
//
if (nHeight >=301 && nHeight <=600)
{
    ret = blockValue * 0.50; 
}
//
if (nHeight >=601 && nHeight <=1000)
{
    ret = blockValue * 0.30; 
}
//
if (nHeight >=1001 && nHeight <=30000)
{
    ret = blockValue * 0.70; 
}
//
if (nHeight >=30001 && nHeight <=35000)
{
    ret = blockValue * 0.60; 
}
//
if (nHeight >=35001 && nHeight <=40000)
{
    ret = blockValue * 0.50; 
}
//
...
void CheckForkWarningConditionsOnNewFork(CBlockIndex* pindexNewForkTip)
{
    ...
    // We use 7 blocks rather XVGDGtrarily as it represents just under 10% of sustained network
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // XVGDG: We use certain sporks during IBD, so check to see if they are
