...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("XVGDGMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("XVGDGMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("XVGDGMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("XVGDG-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running XVGDGMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
